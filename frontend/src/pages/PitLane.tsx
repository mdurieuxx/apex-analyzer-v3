    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {live.lanes.map(lane => (
            <div key={lane.lane} className="bg-gray-900 rounded-lg border border-gray-800 p-3">
              <h3 className="text-xs font-bold text-gray-400 uppercase mb-3">Lane {lane.lane}</h3>
      ...
      <section>
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          ...
          {filter && (
            <span className="text-xs text-gray-500">{filteredHistory.length} result{filteredHistory.length !== 1 ? 's' : ''}</span>
          )}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-900 text-gray-400 text-xs uppercase">
                ...
                <th className="px-3 py-2 text-center">Pit #</th>
                <th className="px-3 py-2 text-right">Duration</th>
                <th className="px-3 py-2 text-right">Pit lap</th>
              ...
              {filteredHistory.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-3 py-6 text-center text-gray-600 text-xs">
                    {filter ? 'No results' : 'No pit stops recorded'}
                  </td>
                </tr>
              )}
            </tbody>
    ...
    <div className={clsx(
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          {eligible
            ? <div className="text-xs text-green-400 flex items-center gap-1 justify-end"><CheckCircle size={12} /> Ready</div>
            : <div className="text-xs text-orange-400 flex items-center gap-1 justify-end"><AlertCircle size={12} /> Min wait</div>
          }
        </div>
      </div>
      ...
      <div className="mt-2 text-xs text-gray-500">Position at pit entry: <span className="text-white">P{ps.position}</span></div>
    </div>
  )
}
