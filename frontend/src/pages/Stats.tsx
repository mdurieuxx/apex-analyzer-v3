...
function FilterBar({ value, onChange, placeholder = 'Filter…', className }: {
...
function EntryPanel({ entryId }: { entryId: number }) {
  ...
  if (!detail) return <div className="p-3 text-xs text-gray-500 animate-pulse">Loading…</div>
      ...
      <div className="flex items-center gap-2 px-3 pt-2 pb-1">
        {(['pilots', 'stints'] as const).map(v => (
          <button key={v} onClick={() => { setView(v); setFilter('') }}
            ...
            {v === 'pilots' ? 'Drivers' : 'Stints'}
          </button>
        ))}
        <FilterBar value={filter} onChange={setFilter} placeholder="Filter driver…" className="flex-1 max-w-xs" />
      </div>
      ...
      {view === 'stints' && (
        <>
          <div className={clsx('px-3 py-1 grid gap-2', STINT_COLS)}>
            ...
            <Hdr col="driver_name" label="Driver" sort={stintSort.sort} onSort={stintSort.toggle} />
            <span className="text-right text-xs text-gray-500 font-medium">Lvl.</span>
            <Hdr col="started_at" label="Time" sort={stintSort.sort} onSort={stintSort.toggle} right />
            <Hdr col="duration" label="Dur." sort={stintSort.sort} onSort={stintSort.toggle} right />
            <Hdr col="lap_count" label="L." sort={stintSort.sort} onSort={stintSort.toggle} right />
            <Hdr col="best_lap_ms" label="Best" sort={stintSort.sort} onSort={stintSort.toggle} right />
            <Hdr col="avg_lap_ms" label="Avg." sort={stintSort.sort} onSort={stintSort.toggle} right />
            <span className="text-right text-xs text-gray-500 font-medium">Kart</span>
          </div>
          {visibleStints.map((s, i) => <StintRow key={s.id} s={s} idx={i} />)}
          {visibleStints.length === 0 && <p className="px-3 py-2 text-xs text-gray-600">No results.</p>}
        </>
      )}
      ...
      {view === 'pilots' && (
        <>
          <div className={clsx('px-3 py-1 grid gap-2', PILOT_COLS)}>
            <Hdr col="driver_name" label="Driver" sort={pilotSort.sort} onSort={pilotSort.toggle} />
            ...
            <Hdr col="avg_lap_ms" label="Avg." sort={pilotSort.sort} onSort={pilotSort.toggle} right />
            <Hdr col="cv" label="σ%" sort={pilotSort.sort} onSort={pilotSort.toggle} right />
            <Hdr col="stars" label="Level" sort={pilotSort.sort} onSort={pilotSort.toggle} right />
          </div>
          ...
          {visiblePilots.length === 0 && <p className="px-3 py-2 text-xs text-gray-600">No results.</p>}
        </>
      )}
    </div>
  )
...
function EventPanel({ eventId }: { eventId: number }) {
  ...
  if (!data) return <div className="p-4 text-sm text-gray-500 animate-pulse">Loading…</div>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-1 border-b border-gray-800 pb-0">
        {(['teams', 'pilots'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'teams' ? <><Users size={13} className="inline mr-1" />Teams</> : <><Flag size={13} className="inline mr-1" />Drivers</>}
          </button>
        ))}
        <span className="ml-auto text-xs text-gray-600 self-center pr-2">{data.entries.length} teams</span>
      </div>
      ...
      {tab === 'teams' && (
        <div className="rounded-lg border border-gray-800 overflow-hidden">
          <div className="px-3 py-1.5 bg-gray-800/40 border-b border-gray-800">
            <FilterBar value={teamFilter} onChange={setTeamFilter} placeholder="Filter team / bib…" />
          </div>
          <div className={clsx('grid gap-2 px-3 py-1.5 bg-gray-800/60', TEAM_COLS)}>
            ...
            <Hdr col="team_name" label="Team" sort={teamSort.sort} onSort={teamSort.toggle} />
            ...
            <Hdr col="avg_lap_ms" label="Avg." sort={teamSort.sort} onSort={teamSort.toggle} right />
            <Hdr col="pit_count" label="Pit" sort={teamSort.sort} onSort={teamSort.toggle} right />
            <Hdr col="stars" label="Level" sort={teamSort.sort} onSort={teamSort.toggle} right />
            <span />
          </div>
          ...
          {visibleTeams.length === 0 && <p className="px-3 py-3 text-sm text-gray-600">No results.</p>}
        </div>
      )}
      ...
      {tab === 'pilots' && (
        <div className="rounded-lg border border-gray-800 overflow-hidden">
          <div className="px-3 py-1.5 bg-gray-800/40 border-b border-gray-800">
            <FilterBar value={pilotFilter} onChange={setPilotFilter} placeholder="Filter driver / team…" />
          </div>
          <div className={clsx('grid gap-2 px-3 py-1.5 bg-gray-800/60', PCOLS)}>
            <Hdr col="driver_name" label="Driver · Team" sort={pilotSort.sort} onSort={pilotSort.toggle} />
            <Hdr col="stint_count" label="St." sort={pilotSort.sort} onSort={pilotSort.toggle} right />
            <Hdr col="total_laps" label="T." sort={pilotSort.sort} onSort={pilotSort.toggle} right />
            <Hdr col="best_lap_ms" label="Best" sort={pilotSort.sort} onSort={pilotSort.toggle} right />
            <Hdr col="avg_lap_ms" label="Avg." sort={pilotSort.sort} onSort={pilotSort.toggle} right />
          ...
          {visiblePilots.length === 0 && <p className="px-3 py-4 text-sm text-gray-600">No results.</p>}
        </div>
      )}
    </div>
  )
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-semibold text-gray-300 flex items-center gap-1.5">
          <Search size={14} /> Search team / driver
        </h2>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Team name or driver…"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
        </div>
        {searchLoading && <p className="text-xs text-gray-500 animate-pulse">Searching…</p>}
            ...
            {searchResults.map((r, i) => (
              <button key={i}
                ...
                <div className="text-right shrink-0 text-xs text-gray-400 space-y-0.5">
                  <p className="font-bold text-white">{r.total_laps} laps</p>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <FilterBar value={eventFilter} onChange={setEventFilter} placeholder="Filter event…" className="flex-1 min-w-[160px] max-w-xs" />
          <div className="flex items-center gap-1 text-xs text-gray-500">
            ...
            {(['event_date', 'name', 'duration_hours'] as const).map(col => (
              <button key={col} onClick={() => eventSort.toggle(col)}
                ...
                {col === 'event_date' ? 'Date' : col === 'name' ? 'Name' : 'Duration'}
        ...
        {filteredEvents.length === 0 && (
          <p className="text-sm text-gray-600">No imported events.</p>
        )}
              ...
              <button
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {ev.duration_hours}h
                    {ev.event_date && ` · ${new Date(ev.event_date).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}`}
                  </p>
                </div>
                {selectedEvent === ev.id
